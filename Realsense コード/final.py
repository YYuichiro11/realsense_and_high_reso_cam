...
while True:
    ...
    if pose_frame_2:
        ...
        array_data2 = [2,pose_frame_2.frame_number,x2+fx2,y2+fy2,z2,omg2,phi2,kpp2,depth2]
    ...
    if pose_frame_3:
        ...
        array_data3 = [3,pose_frame_3.frame_number,x3+fx3,y3+fy3,z3,omg3,phi3,kpp3,depth3]
    ...
    if abs(t1[0]) < t_err and abs(t1[1]) < t_err and abs(t1[2]) < t_err:
        ...
        if feature_points2.size >= feature_points3.size:
            if abs(t2[0]) < t_err and abs(t2[1]) < t_err and abs(t2[2]) < t_err:
                position = position + t2
                id_position = np.insert(position,0,i)
                positions = np.vstack((positions,id_position))
            else:
                if abs(t3[0]) < t_err and abs(t3[1]) < t_err and abs(t3[2]) < t_err:
                    ...
                    t1_1 = t1
                    k = 0
                    while abs(t1_1[0]) > t_err or abs(t1_1[1]) > t_err or abs(t1_1[2]) > t_err:
                        j = j + 1
                        t1_1 = t1_1/j
                    for k in range(j):
                        k = k+1
                        position = position + t1_1
                        l = i-1+k
                        id_position = np.insert(position,0,l)
                        positions = np.vstack((positions,id_position))
        else:
            if abs(t3[0]) < t_err and abs(t3[1]) < t_err and abs(t3[2]) < t_err:
                position = position + t2
                ...
                if abs(t2[0]) < t_err and abs(t2[1]) < t_err and abs(t2[2]) < t_err:
                    ...
                    t1_1 = t1
                    k = 0
                    while abs(t1_1[0]) > t_err or (t1_1[1]) > t_err or t1_1[2] > t_err:
                        j = j+1
                        t1_1 = t1_1/j
                    for k in range(j):
                        k = k+1
