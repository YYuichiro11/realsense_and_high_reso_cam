...
def image_vcombine(im_info1, im_info2):
    ...
    new_img = cv2.resize(np.zeros((1, 1, 3), np.uint8), (size,h))
    
    new_img[:, start:fin] = old_img
    
    img = cv2.vconcat([img1,new_img])          # 2つの画像を横方向に連結
    return img

# 動画を空間方向に連結させる関数
def m_space_combine(movie1, movie2, path_out, scale_factor):
...
# ここからメイン実行文
movie1 = ['C:/graduation invastigation/information/camera1_4.mp4', True]     # 元動画のパス1, カラーはTrue
movie2 = ['C:/graduation invastigation/information/tracking1_4.mp4', True]    # 元動画のパス2, 白黒はFalse
path_out = 'C:/graduation invastigation/information/movie_out1.mp4'        # 保存する動画のパス
scale_factor = 1                  # FPSにかけるスケールファクター
 
# 複数動画を連結させる関数を実行
m_space_combine(movie1, movie2, path_out, scale_factor)